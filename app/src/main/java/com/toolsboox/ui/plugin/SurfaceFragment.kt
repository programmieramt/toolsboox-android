...
abstract class SurfaceFragment : ScreenFragment() {
    ...
    private fun onMoveDrawing(touchPoints: List<StrokePoint>) {
        ...
        if (touchHelper == null && penState) {
            ...
            if (lockCanvas != null) {
                ...
                lockCanvas.drawPath(path, livePreviewPaint())
                lockCanvas.restore()
                provideSurfaceView().holder.unlockCanvasAndPost(lockCanvas)
            }
        }
    ...
    /** Lazily-built non-AA paint for the no-Onyx software live preview (matches [paint] width/color). */
    private var _livePreviewPaint: Paint? = null
    private fun livePreviewPaint(): Paint {
        val p = _livePreviewPaint ?: Paint().also { _livePreviewPaint = it }
