...
5. **Note page OCR** — extra note pages get their own freeform Sonnet pass and append as "Page N" sections to the daily Ledger note on mjh.yoga
6. **Multi-device merge** — write on any Boox throughout the day (Tab8, Palma 2 Pro, Tab Mini C, etc.); the processor walks all device folders on WebDAV, dedupes strokes by ID, and consolidates everything into one Ledger note per date
